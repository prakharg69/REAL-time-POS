...
const CheckoutPage = () => {
  ...
  const handleCheckout = () => {
    if (!cartData.totalQuantity) return;
        ...
        <div className="max-w-7xl mx-auto px-4 py-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-2">
          <div>
            ...
            <p className="text-blue-200 text-sm mt-1">
              Store: {Store?.shopName || "Demo Store"}{" "}
              <span className="mx-2 text-blue-700">|</span> ID:{" "}
              {Store?._id ? Store._id.substring(0, 8) + "..." : "..."}
            </p>
          </div>
          ...
          <div className="space-y-3">
            {Object.values(cartData.items).length > 0 ? (
              Object.values(cartData.items).map((item) => (
                <div
                  key={item.sku}
                  className="bg-white rounded border border-slate-200 p-0 overflow-hidden grid grid-cols-1 sm:grid-cols-12 gap-0"
                >
                  {/* Item Info (Left) */}
                      ...
                      <div className="mt-3 flex items-center gap-3">
                        <div className="flex items-center border border-slate-300 rounded bg-slate-50">
                          <button
                            ...
                            className="px-2 py-1 text-slate-600 font-bold hover:bg-slate-200 border-r border-slate-300"
                          >
                            −
                          </button>
                          <span className="px-3 py-1 text-sm font-semibold text-slate-700 min-w-12 text-center">
                            {item.quantity}
                          </span>
                          <button
                            onClick={() => handleIncrement(item.sku)}
                            className="px-2 py-1 text-slate-600 font-bold hover:bg-slate-200 border-l border-slate-300"
                          >
                            +
                          </button>
                        </div>
              ...
              <div className="p-6 flex flex-col items-center justify-center bg-slate-100/50 relative">
                ...
                <div className="absolute inset-4 border-2 border-blue-100 rounded-lg pointer-events-none flex flex-col justify-between p-2">
                  <div className="flex justify-between">
                    <div className="w-4 h-4 border-t-4 border-l-4 border-blue-500 rounded-tl-sm"></div>
                    <div className="w-4 h-4 border-t-4 border-r-4 border-blue-500 rounded-tr-sm"></div>
                  </div>
                  <div className="flex justify-between">
                    <div className="w-4 h-4 border-b-4 border-l-4 border-blue-500 rounded-bl-sm"></div>
                    <div className="w-4 h-4 border-b-4 border-r-4 border-blue-500 rounded-br-sm"></div>
                  </div>
                </div>
